...
class EncLic:
    ...
    def enc_lic(self):
        ...
        # 填充 ori key
        ori_key = pad(self.key, AES.block_size)
        # ori_lic = Utils.add_to_16(Utils.format_license(self.lic, self.due_time, self.remark))
        # 填充 lic
        ori_lic = pad(Utils.format_license(self.lic, self.due_time, self.remark), AES.block_size)
...
if __name__ == '__main__':
    key_gen = GenKey('333', 'zhouqin')
    sss = key_gen.gen_keys()
    print(sss)

    lic_enc = EncLic(
        './data/20230727/333/20230727_333_key.ori'
        , './data/20230727/333/License.lic'
        , '2023-11-13 13:11:20', None)
    print(lic_enc.lic)
    print(lic_enc.key)
